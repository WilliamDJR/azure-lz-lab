...
This chapter maps reusable CI/CD platform concepts to Azure Pipelines and explains the syntax and operating controls that differ from GitHub Actions. Runnable examples are in `azure-devops/`.
...
## 2. Core Azure Pipelines concepts
...
Syntax translation is only one part of a migration. The long-term value comes from replacing many hand-maintained pipelines with a small set of versioned, governed templates.
...
Azure DevOps remains a supported enterprise platform, while the newest agentic source-code workflows arrive first on GitHub.
...
### Criteria for evaluating an organization's direction
...
Large-scale CI/CD migration requires template design, versioning, parallel validation, and controlled rollout in addition to syntax translation.

### Recommended learning scope

The recommended scope covers the ability to:
...
Follow [06-alz-accelerator.md](06-alz-accelerator.md) after this lab. Compare the controls, but do not overwrite the generated Accelerator repository with this sample pipeline; upgrade and operate the official generated repository as the platform source of truth.
